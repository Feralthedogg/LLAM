...
static int test_native_storage_is_lazy_and_bounded(void) {
    int failed = 0;

    if (leir_native_instance_size() > 384U) {
        fprintf(stderr, "common native instance exceeded 384 bytes\n");
        return 1;
    }
#if defined(__linux__)
    bind_fixture_t fixture;
    const void *linux_state;

    if (leir_native_test_linux_core_size() == 0U ||
        leir_native_test_linux_core_size() > 896U ||
        leir_native_test_linux_fixed_size() == 0U ||
        leir_native_test_linux_fixed_size() > 512U) {
        fprintf(stderr, "Linux native cold-state budget exceeded\n");
        return 1;
    }
    if (bind_fixture_init(&fixture, false) != 0) {
        perror("lazy native storage fixture init");
        return 1;
    }
    if (leir_native_test_linux_state_address(
            fixture.instance) != NULL ||
        leir_native_test_linux_fixed_address(
            fixture.instance) != NULL) {
        fprintf(stderr, "native cold state was allocated during init\n");
        failed = 1;
        goto done;
    }
    fixture.values[TEST_LENGTH_SLOT].u64 = 0U;
    if (expect_bind_failure(&fixture, EINVAL) != 0 ||
        leir_native_test_linux_state_address(
            fixture.instance) != NULL) {
        fprintf(stderr, "failed initial bind retained cold state\n");
        failed = 1;
        goto done;
    }
    ...
    if (leir_native_instance_bind(
        ...
        perror("lazy native storage bind");
        failed = 1;
        goto done;
    }
    linux_state = leir_native_test_linux_state_address(
        fixture.instance);
    if (linux_state == NULL ||
        leir_native_test_linux_fixed_address(
            fixture.instance) != NULL) {
        fprintf(stderr, "direct mode cold state was not split\n");
        failed = 1;
        goto done;
    }
    fixture.values[TEST_LENGTH_SLOT].u64 = 0U;
    if (expect_bind_failure(&fixture, EINVAL) != 0 ||
        leir_native_test_linux_state_address(
            fixture.instance) != linux_state) {
        fprintf(stderr, "failed rebind replaced prior Linux state\n");
        failed = 1;
    }
    ...
    if (bind_fixture_init_mode(
        ...
        leir_native_instance_bind(
            ...
            fixture.values,
            fixture.value_count) != 0) {
        perror("fixed lazy native storage bind");
        bind_fixture_destroy(&fixture);
        return 1;
    }
    if (leir_native_test_linux_state_address(
        ...
        leir_native_test_linux_fixed_address(
            fixture.instance) == NULL) {
        fprintf(stderr, "fixed mode did not allocate split fixed state\n");
        failed = 1;
    }
    bind_fixture_destroy(&fixture);
#else
    if (leir_native_test_linux_core_size() != 0U ||
        leir_native_test_linux_fixed_size() != 0U) {
        fprintf(stderr, "non-Linux build reported Linux cold storage\n");
        failed = 1;
    }
#endif
    return failed;
...
int main(void) {
    ...
    if (test_init_validates_storage() != 0 ||
        test_native_storage_is_lazy_and_bounded() != 0 ||
