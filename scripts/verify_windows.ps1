...
if ($Native) {
    ...
    & ctest --test-dir $buildDir --output-on-failure --timeout 180 -C Release
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
    ...
    foreach ($forcedGeneration in @("10", "11")) {
        ...
        & ctest --test-dir $buildDir --output-on-failure --timeout 180 -C Release -R "test_abi_compat|test_multi_runtime_core|test_runtime_stress|test_runtime_fuzz|test_runtime_invariants|test_runtime_shutdown_internal|test_windows_policy|test_windows_runtime_smoke|test_windows_iocp_io|test_windows_handle_io|test_security_capability|llam_broker_self_test"
        if ($LASTEXITCODE -ne 0) {
