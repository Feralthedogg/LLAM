...
target="${1:-}"
version="${LLAM_RELEASE_VERSION:-${GITHUB_REF_NAME:-v1.0.2}}"
version="${version#v}"
version="$(printf '%s' "$version" | tr '/' '-')"
abi_major="${LLAM_ABI_MAJOR:-1}"
library_version="${LLAM_VERSION:-1.0.2}"
...
require_input "$root_dir/examples/server_support.c"
require_input "$root_dir/examples/server_support.h"
...
cp "$root_dir/examples/demo.c" "$root_dir/examples/bench.c" "$root_dir/examples/stress.c" "$root_dir/examples/server.c" "$root_dir/examples/server_support.c" "$root_dir/examples/server_support.h" "$root_dir/examples/server_flood.c" "$stage/examples/"
