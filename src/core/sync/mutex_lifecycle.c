/**
 * @file src/core/sync/mutex_lifecycle.c
 ...
 * rejection, and create/destroy paths; mutex.c keeps the hot
...
llam_mutex_t *llam_mutex_resolve_public_handle(const llam_mutex_t *handle) {
    ...
    if (mutex == NULL) {
        errno = EINVAL;
    } else if (llam_public_active_op_try_begin(&mutex->active_ops) != 0) {
        mutex = NULL;
    }
    pthread_mutex_unlock(&g_llam_mutex_registry_lock);
    return mutex;
