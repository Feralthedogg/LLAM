/**
 * @file src/io/runtime_io_watch_darwin_migration_live.c
...
bool nm_forward_live_poll_watch_event(nm_node_t *source,
    ...
    if (target_watch->wait_head != NULL) {
        ...
        if (!nm_darwin_poll_completion_push(&poll_completions, &poll_completion_tail, waiters, merged_revents)) {
            if (waiters != NULL) {
...
bool nm_forward_live_accept_watch_ready(nm_node_t *source, int fd, unsigned target_index, int accepted_fd) {
    ...
    if (target_watch->wait_head != NULL) {
        ...
        if (waiter == NULL ||
            !nm_darwin_accept_completion_push(&accept_completions, &accept_completion_tail, waiter, accepted_fd)) {
            if (waiter != NULL) {
...
bool nm_forward_live_recv_watch_ready(nm_node_t *source,
    ...
    if (target_watch->wait_head != NULL) {
        ...
        if (waiter == NULL ||
            !nm_darwin_recv_completion_push_copy(&recv_completions,
    ...
    if (!nm_recv_watch_push_ready_copy(target_watch, data, size)) {
        goto out;
    }
    ok = true;
