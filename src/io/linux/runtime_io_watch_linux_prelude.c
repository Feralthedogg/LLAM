/**
 * @file src/io/linux/runtime_io_watch_linux_prelude.c
...
bool llam_node_spin_for_cqe(llam_node_t *node) {
    ...
    for (iters = 0; iters < LLAM_IO_SQPOLL_SPIN_ITERS; ++iters) {
        if (io_uring_cq_ready(&node->ring) > 0U) {
            llam_io_drain_completions(node);
            return true;
        }
        if (atomic_load(&node->pending_ops) == 0U || llam_now_ns() - start_ns >= LLAM_IO_SQPOLL_SPIN_NS) {
            break;
        }
        llam_pause_cpu();
    }
