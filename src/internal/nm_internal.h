/**
 * @file src/internal/nm_internal.h
 * @brief Primary internal include that joins runtime types, state, platform, and subsystem prototypes.
 *
 * @details
 * This header defines the minimal ABI shared with hand-written context-switch
 * assembly before the larger runtime type graph is included. The architecture
 * specific ::nm_ctx_t layout must stay in lockstep with the assembly files named
 * in the static assertions.
 *
 * @copyright Copyright 2026 Feralthedogg
 *
 * @par License
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef NM_INTERNAL_H
#define NM_INTERNAL_H

#include "llam/nm_runtime.h"

#include <stddef.h>
#include <stdint.h>

#if defined(__linux__) && defined(__x86_64__)
/** @brief Internal task states used by tracing, diagnostics, and assembly-visible code. */
typedef enum nm_task_state_id {
    NM_TASK_STATE_NEW = 0,
    NM_TASK_STATE_RUNNABLE = 1,
    NM_TASK_STATE_RUNNING = 2,
    NM_TASK_STATE_PARKED = 3,
    NM_TASK_STATE_BLOCKED_OPAQUE = 4,
    NM_TASK_STATE_DEAD = 5,
} nm_task_state_id_t;

/** @brief Reason a task left running state or was woken. */
typedef enum nm_wait_reason {
    NM_WAIT_NONE = 0,
    NM_WAIT_YIELD = 1,
    NM_WAIT_JOIN = 2,
    NM_WAIT_SLEEP = 3,
    NM_WAIT_BLOCKING = 4,
    NM_WAIT_IO = 5,
    NM_WAIT_CANCEL = 6,
    NM_WAIT_MUTEX = 7,
    NM_WAIT_COND = 8,
    NM_WAIT_CHANNEL_SEND = 9,
    NM_WAIT_CHANNEL_RECV = 10,
    NM_WAIT_TIMEOUT = 11,
} nm_wait_reason_t;

/** @brief Trace event categories recorded by shard-local trace buffers. */
typedef enum nm_trace_kind {
    NM_TRACE_STATE = 1,
    NM_TRACE_WAKE = 2,
    NM_TRACE_BLOCK_SUBMIT = 3,
    NM_TRACE_BLOCK_COMPLETE = 4,
    NM_TRACE_IO_SUBMIT = 5,
    NM_TRACE_IO_COMPLETE = 6,
    NM_TRACE_IDLE = 7,
    NM_TRACE_STEAL = 8,
    NM_TRACE_WATCHDOG = 9,
} nm_trace_kind_t;

/**
 * @brief Linux x86-64 callee-saved context layout.
 *
 * The offsets below are consumed directly by
 * @c asm/linux/x86_64/context_x86_64.S.
 */
typedef struct nm_ctx {
    uint64_t rsp;
    uint64_t rbx;
    uint64_t rbp;
    uint64_t r12;
    uint64_t r13;
    uint64_t r14;
    uint64_t r15;
    uint32_t mxcsr;
    uint16_t x87_cw;
    uint16_t pad;
    void *xsave_area;
} nm_ctx_t;

_Static_assert(offsetof(nm_ctx_t, rsp) == 0, "nm_ctx_t.rsp offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, rbx) == 8, "nm_ctx_t.rbx offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, rbp) == 16, "nm_ctx_t.rbp offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, r12) == 24, "nm_ctx_t.r12 offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, r13) == 32, "nm_ctx_t.r13 offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, r14) == 40, "nm_ctx_t.r14 offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, r15) == 48, "nm_ctx_t.r15 offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, mxcsr) == 56, "nm_ctx_t.mxcsr offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, x87_cw) == 60, "nm_ctx_t.x87_cw offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(offsetof(nm_ctx_t, xsave_area) == 64, "nm_ctx_t.xsave_area offset must match asm/linux/x86_64/context_x86_64.S");
_Static_assert(sizeof(nm_ctx_t) == 72, "nm_ctx_t size must match asm/linux/x86_64/context_x86_64.S");
#elif defined(__aarch64__)
/** @brief Internal task states used by tracing and diagnostics on AArch64. */
typedef enum nm_task_state_id {
    NM_TASK_STATE_NEW = 0,
    NM_TASK_STATE_RUNNABLE = 1,
    NM_TASK_STATE_RUNNING = 2,
    NM_TASK_STATE_PARKED = 3,
    NM_TASK_STATE_BLOCKED_OPAQUE = 4,
    NM_TASK_STATE_DEAD = 5,
} nm_task_state_id_t;

/** @brief Reason a task left running state or was woken on AArch64. */
typedef enum nm_wait_reason {
    NM_WAIT_NONE = 0,
    NM_WAIT_YIELD = 1,
    NM_WAIT_JOIN = 2,
    NM_WAIT_SLEEP = 3,
    NM_WAIT_BLOCKING = 4,
    NM_WAIT_IO = 5,
    NM_WAIT_CANCEL = 6,
    NM_WAIT_MUTEX = 7,
    NM_WAIT_COND = 8,
    NM_WAIT_CHANNEL_SEND = 9,
    NM_WAIT_CHANNEL_RECV = 10,
    NM_WAIT_TIMEOUT = 11,
} nm_wait_reason_t;

/** @brief Trace event categories recorded by shard-local trace buffers on AArch64. */
typedef enum nm_trace_kind {
    NM_TRACE_STATE = 1,
    NM_TRACE_WAKE = 2,
    NM_TRACE_BLOCK_SUBMIT = 3,
    NM_TRACE_BLOCK_COMPLETE = 4,
    NM_TRACE_IO_SUBMIT = 5,
    NM_TRACE_IO_COMPLETE = 6,
    NM_TRACE_IDLE = 7,
    NM_TRACE_STEAL = 8,
    NM_TRACE_WATCHDOG = 9,
} nm_trace_kind_t;

/**
 * @brief AArch64 callee-saved context layout.
 *
 * Integer callee-saved registers, FP/LR, SIMD d8-d15, and FP control/status are
 * stored in the exact order expected by the AArch64 context switch assembly.
 */
typedef struct nm_ctx {
    uint64_t sp;
    uint64_t x19;
    uint64_t x20;
    uint64_t x21;
    uint64_t x22;
    uint64_t x23;
    uint64_t x24;
    uint64_t x25;
    uint64_t x26;
    uint64_t x27;
    uint64_t x28;
    uint64_t fp;
    uint64_t lr;
    uint64_t d8_bits;
    uint64_t d9_bits;
    uint64_t d10_bits;
    uint64_t d11_bits;
    uint64_t d12_bits;
    uint64_t d13_bits;
    uint64_t d14_bits;
    uint64_t d15_bits;
    uint64_t fpcr;
    uint64_t fpsr;
} nm_ctx_t;

_Static_assert(offsetof(nm_ctx_t, sp) == 0, "nm_ctx_t.sp offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, x19) == 8, "nm_ctx_t.x19 offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, x28) == 80, "nm_ctx_t.x28 offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, fp) == 88, "nm_ctx_t.fp offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, lr) == 96, "nm_ctx_t.lr offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, d8_bits) == 104, "nm_ctx_t.d8_bits offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, d15_bits) == 160, "nm_ctx_t.d15_bits offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, fpcr) == 168, "nm_ctx_t.fpcr offset must match asm/arm64 context switch");
_Static_assert(offsetof(nm_ctx_t, fpsr) == 176, "nm_ctx_t.fpsr offset must match asm/arm64 context switch");
_Static_assert(sizeof(nm_ctx_t) == 184, "nm_ctx_t size must match asm/arm64 context switch");
#else
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
#endif
#include <fenv.h>
#include <ucontext.h>
#if defined(__clang__)
#pragma clang diagnostic pop
#endif

/** @brief Internal task states used by the portable context backend. */
typedef enum nm_task_state_id {
    NM_TASK_STATE_NEW = 0,
    NM_TASK_STATE_RUNNABLE = 1,
    NM_TASK_STATE_RUNNING = 2,
    NM_TASK_STATE_PARKED = 3,
    NM_TASK_STATE_BLOCKED_OPAQUE = 4,
    NM_TASK_STATE_DEAD = 5,
} nm_task_state_id_t;

/** @brief Reason a portable-backend task left running state or was woken. */
typedef enum nm_wait_reason {
    NM_WAIT_NONE = 0,
    NM_WAIT_YIELD = 1,
    NM_WAIT_JOIN = 2,
    NM_WAIT_SLEEP = 3,
    NM_WAIT_BLOCKING = 4,
    NM_WAIT_IO = 5,
    NM_WAIT_CANCEL = 6,
    NM_WAIT_MUTEX = 7,
    NM_WAIT_COND = 8,
    NM_WAIT_CHANNEL_SEND = 9,
    NM_WAIT_CHANNEL_RECV = 10,
    NM_WAIT_TIMEOUT = 11,
} nm_wait_reason_t;

/** @brief Trace event categories recorded by shard-local trace buffers. */
typedef enum nm_trace_kind {
    NM_TRACE_STATE = 1,
    NM_TRACE_WAKE = 2,
    NM_TRACE_BLOCK_SUBMIT = 3,
    NM_TRACE_BLOCK_COMPLETE = 4,
    NM_TRACE_IO_SUBMIT = 5,
    NM_TRACE_IO_COMPLETE = 6,
    NM_TRACE_IDLE = 7,
    NM_TRACE_STEAL = 8,
    NM_TRACE_WATCHDOG = 9,
} nm_trace_kind_t;

/**
 * @brief Portable ucontext-backed context layout.
 *
 * Used only when no architecture-specific assembly context switch is selected.
 */
typedef struct nm_ctx {
    ucontext_t uc;
    fenv_t fenv;
    int fenv_valid;
} nm_ctx_t;
#endif

/*
 * Context-switch and fiber bootstrap ABI.
 *
 * These functions are implemented either by assembly or the portable ucontext
 * backend. The noreturn functions terminate a task stack back into scheduler
 * code and must never return to their caller.
 */
void nm_ctx_switch(nm_ctx_t *from, const nm_ctx_t *to);
void nm_fiber_bootstrap(void);
void nm_fiber_alignment_violation(uint64_t rsp) __attribute__((noreturn));
void nm_task_bootstrap(struct nm_task *task) __attribute__((noreturn));
void nm_task_exit_internal(void) __attribute__((noreturn));
int nm_ctx_make_task(nm_ctx_t *ctx, void *stack_base, size_t stack_size, struct nm_task *task);

/*
 * Diagnostic formatting helpers shared by dumps, traces, and examples.
 */
const char *nm_state_name_from_id(nm_task_state_id_t state);
const char *nm_wait_reason_name(nm_wait_reason_t reason);
size_t nm_stack_bytes(nm_stack_class_t stack_class);

#endif
