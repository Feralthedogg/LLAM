...
NM_API const char *nm_version_string(void);
NM_API int nm_abi_get_info(nm_abi_info_t *info, size_t info_size);
...
NM_API int nm_runtime_opts_init(nm_runtime_opts_t *opts, size_t opts_size);
NM_API int nm_spawn_opts_init(nm_spawn_opts_t *opts, size_t opts_size);
NM_API int nm_runtime_init_ex(const nm_runtime_opts_t *opts, size_t opts_size);
NM_API int nm_runtime_init(const nm_runtime_opts_t *opts);
NM_API int nm_runtime_request_stop(void);
NM_API void nm_runtime_shutdown(void);
NM_API int nm_runtime_collect_stats_ex(nm_runtime_stats_t *stats, size_t stats_size);
NM_API int nm_runtime_collect_stats(nm_runtime_stats_t *stats);
NM_API int nm_runtime_write_stats_json(int fd);

NM_API nm_task_t *nm_spawn_ex(nm_task_fn fn, void *arg, const nm_spawn_opts_t *opts, size_t opts_size);
NM_API nm_task_t *nm_spawn(nm_task_fn fn, void *arg, const nm_spawn_opts_t *opts);
NM_API int nm_run(void);
NM_API void nm_yield(void);
NM_API void nm_task_safepoint(void);
NM_API int nm_join(nm_task_t *task);
NM_API int nm_join_until(nm_task_t *task, uint64_t deadline_ns);
NM_API int nm_detach(nm_task_t *task);
NM_API int nm_sleep_until(uint64_t deadline_ns);
NM_API int nm_sleep_ns(uint64_t duration_ns);
NM_API int nm_call_blocking_result(nm_blocking_fn fn, void *arg, void **out);
NM_API void *nm_call_blocking(nm_blocking_fn fn, void *arg);
NM_API int nm_enter_blocking(void);
NM_API int nm_leave_blocking(void);
NM_API int nm_task_set_class(uint32_t task_class);
NM_API void nm_dump_runtime_state(int fd);
NM_API uint32_t nm_task_flags(const nm_task_t *task);

/* Cancellation tokens. */
NM_API nm_cancel_token_t *nm_cancel_token_create(void);
NM_API int nm_cancel_token_destroy(nm_cancel_token_t *token);
NM_API int nm_cancel_token_cancel(nm_cancel_token_t *token);
NM_API int nm_cancel_token_is_cancelled(const nm_cancel_token_t *token);

/* Runtime-aware mutexes. */
NM_API nm_mutex_t *nm_mutex_create(void);
NM_API int nm_mutex_destroy(nm_mutex_t *mutex);
NM_API int nm_mutex_lock(nm_mutex_t *mutex);
NM_API int nm_mutex_lock_until(nm_mutex_t *mutex, uint64_t deadline_ns);
NM_API int nm_mutex_trylock(nm_mutex_t *mutex);
NM_API int nm_mutex_unlock(nm_mutex_t *mutex);

/* Runtime-aware condition variables. */
NM_API nm_cond_t *nm_cond_create(void);
NM_API int nm_cond_destroy(nm_cond_t *cond);
NM_API int nm_cond_wait(nm_cond_t *cond, nm_mutex_t *mutex);
NM_API int nm_cond_wait_until(nm_cond_t *cond, nm_mutex_t *mutex, uint64_t deadline_ns);
NM_API int nm_cond_signal(nm_cond_t *cond);
NM_API int nm_cond_broadcast(nm_cond_t *cond);

/* Bounded pointer channels. Capacity must be at least 1. */
NM_API nm_channel_t *nm_channel_create(size_t capacity);
NM_API int nm_channel_destroy(nm_channel_t *channel);
NM_API int nm_channel_send(nm_channel_t *channel, void *value);
NM_API int nm_channel_send_until(nm_channel_t *channel, void *value, uint64_t deadline_ns);
NM_API int nm_channel_recv_result(nm_channel_t *channel, void **out);
NM_API int nm_channel_recv_until_result(nm_channel_t *channel, uint64_t deadline_ns, void **out);
NM_API void *nm_channel_recv(nm_channel_t *channel);
NM_API void *nm_channel_recv_until(nm_channel_t *channel, uint64_t deadline_ns);
NM_API int nm_channel_close(nm_channel_t *channel);

/* Runtime-integrated I/O helpers. */
NM_API ssize_t nm_read(nm_fd_t fd, void *buf, size_t count);
NM_API ssize_t nm_read_when_ready(nm_fd_t fd, void *buf, size_t count, int timeout_ms);
NM_API ssize_t nm_write(nm_fd_t fd, const void *buf, size_t count);
NM_API ssize_t nm_read_owned(nm_fd_t fd, size_t max_count, nm_io_buffer_t **out);
NM_API ssize_t nm_recv_owned(nm_fd_t fd, size_t max_count, int flags, nm_io_buffer_t **out);
NM_API void nm_io_buffer_release(nm_io_buffer_t *buffer);
NM_API void *nm_io_buffer_data(nm_io_buffer_t *buffer);
NM_API size_t nm_io_buffer_size(const nm_io_buffer_t *buffer);
NM_API size_t nm_io_buffer_capacity(const nm_io_buffer_t *buffer);
/** @brief Accept a connection, returning @c NM_INVALID_FD on failure with @c errno set. */
NM_API nm_fd_t nm_accept(nm_fd_t fd, struct sockaddr *addr, socklen_t *addrlen);
NM_API int nm_connect(nm_fd_t fd, const struct sockaddr *addr, socklen_t addrlen);
NM_API int nm_poll_fd(nm_fd_t fd, short events, int timeout_ms, short *revents);

/* Time and task introspection. */
NM_API uint64_t nm_now_ns(void);
NM_API uint64_t nm_task_id(const nm_task_t *task);
NM_API const char *nm_task_state_name(const nm_task_t *task);
NM_API uint32_t nm_task_class(const nm_task_t *task);
NM_API nm_task_t *nm_current_task(void);
