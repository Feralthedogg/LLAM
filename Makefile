...
CLEAN_FILES = \
	...
	test_lcrs_topology.exe \
	bench_lcrs_model.exe \
...
LCRS_MODEL_CORE_OBJS = \
	$(OBJDIR)/experiments/lcrs/lcrs_topology.o \
	$(OBJDIR)/experiments/lcrs/lcrs_simulator.o
LCRS_MODEL_TEST_OBJS = \
	$(OBJDIR)/experiments/lcrs/test_lcrs_topology.o
LCRS_MODEL_BENCH_OBJS = \
	$(OBJDIR)/experiments/lcrs/bench_lcrs_model.o
...
WINDOWS_CTEST_REGEX ?= test_abi_contract|test_abi_compat|test_runtime_core|test_multi_runtime_core|test_runtime_api_edges|test_runtime_select_edges|test_runtime_group_local_edges|test_runtime_unmanaged_join|test_runtime_stress|test_runtime_fuzz|test_runtime_invariants|test_runtime_shutdown_internal|test_sync_primitives|test_windows_policy|test_windows_runtime_smoke|test_windows_iocp_io|test_windows_iocp_dump|test_windows_handle_io|test_security_capability|test_lcwe_model|test_bench_lcwe_model|test_lcrs_topology|test_bench_lcrs_model|llam_broker_self_test
...
test-lcrs-model: windows-cmake-configure
	cmake --build "$(WINDOWS_CMAKE_BUILD_DIR)" --config "$(WINDOWS_CMAKE_CONFIG)" --target test_lcrs_topology bench_lcrs_model
	ctest --test-dir "$(WINDOWS_CMAKE_BUILD_DIR)" --output-on-failure -C "$(WINDOWS_CMAKE_CONFIG)" -R "test_lcrs_topology|test_bench_lcrs_model" $(WINDOWS_CTEST_ARGS)
...
test-lcrs-model: test_lcrs_topology bench_lcrs_model
	./test_lcrs_topology
	LCRS_MODEL_TEST_BINARY=./bench_lcrs_model python3 scripts/test_bench_lcrs_model.py

lcrs-model-report: test-lcrs-model
	python3 scripts/bench_lcrs_model.py \
		--binary ./bench_lcrs_model \
...
test_lcrs_topology: $(LCRS_MODEL_CORE_OBJS) $(LCRS_MODEL_TEST_OBJS)
	$(CC) $(CFLAGS) -o $@ $(LCRS_MODEL_CORE_OBJS) $(LCRS_MODEL_TEST_OBJS) $(SERVER_FLOOD_LDLIBS)

bench_lcrs_model: $(LCRS_MODEL_CORE_OBJS) $(LCRS_MODEL_BENCH_OBJS)
	$(CC) $(CFLAGS) -o $@ $(LCRS_MODEL_CORE_OBJS) $(LCRS_MODEL_BENCH_OBJS) $(SERVER_FLOOD_LDLIBS)
...
$(OBJDIR)/experiments/lcrs/%.o: experiments/lcrs/%.c \
		experiments/lcrs/lcrs_model.h
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CFLAGS) -Iexperiments/lcrs -c -o $@ $<
