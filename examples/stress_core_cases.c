...
void run_channel_ping_pong(void) {
    ...
    state.request = nm_channel_create(1U);
    state.response = nm_channel_create(1U);
    state.rounds = 64U;
    atomic_init(&state.echoed, 0U);
    if (state.request == NULL || state.response == NULL) {
        stress_fail_msg("ping channels create failed");
        if (state.request != NULL) {
            nm_channel_destroy(state.request);
        }
        if (state.response != NULL) {
            nm_channel_destroy(state.response);
        }
        return;
    }

    peer = nm_spawn(ping_peer_task,
                    &state,
                    &(nm_spawn_opts_t){
                        .task_class = NM_TASK_CLASS_DEFAULT,
                        .stack_class = NM_STACK_CLASS_DEFAULT,
    ...
    for (i = 0; i < state.rounds; ++i) {
        ...
        if (nm_channel_send(state.request, token) != 0) {
            stress_fail_msg("ping request send failed");
            break;
        }
        echo = nm_channel_recv(state.response);
        if (echo != token) {
...
void run_mutex_convoy(void) {
    ...
    for (i = 0; i < state.workers; ++i) {
        tasks[i] = nm_spawn(convoy_worker_task,
                            &state,
                            &(nm_spawn_opts_t){
                                .task_class = NM_TASK_CLASS_DEFAULT,
                                .stack_class = NM_STACK_CLASS_DEFAULT,
...
void run_cancel_path(void) {
    ...
    waiter = nm_spawn(cancel_waiter_task,
                      &state,
                      &(nm_spawn_opts_t){
                          .task_class = NM_TASK_CLASS_DEFAULT,
                          .stack_class = NM_STACK_CLASS_DEFAULT,
    ...
    trigger = nm_spawn(cancel_trigger_task,
                       &state,
                       &(nm_spawn_opts_t){
                           .task_class = NM_TASK_CLASS_DEFAULT,
                           .stack_class = NM_STACK_CLASS_DEFAULT,
...
void run_io_cancel_path(void) {
    ...
    waiter = nm_spawn(io_cancel_waiter_task,
                      &state,
                      &(nm_spawn_opts_t){
                          .task_class = NM_TASK_CLASS_DEFAULT,
                          .stack_class = NM_STACK_CLASS_DEFAULT,
    ...
    trigger = nm_spawn(io_cancel_trigger_task,
                       &state,
                       &(nm_spawn_opts_t){
                           .task_class = NM_TASK_CLASS_DEFAULT,
                           .stack_class = NM_STACK_CLASS_DEFAULT,
...
void run_owned_read_paths(void) {
    ...
    small_state.fd = sv[1];
    writer = nm_spawn(owned_read_writer_task,
                      &small_state,
                      &(nm_spawn_opts_t){
                          .task_class = NM_TASK_CLASS_DEFAULT,
                          .stack_class = NM_STACK_CLASS_DEFAULT,
    ...
    rc = nm_read_owned(sv[0], 16U, &buffer);
    if (rc != (ssize_t)small_state.len) {
        stress_fail_msg("owned read small rc failed");
    } else if (buffer == NULL || nm_io_buffer_capacity(buffer) < small_state.len) {
        stress_fail_msg("owned read small capacity failed");
    } else if (memcmp(nm_io_buffer_data(buffer), small_state.payload, small_state.len) != 0) {
    ...
    rc = nm_read_owned(sv[0], kLargeOwnedBytes, &buffer);
    if (rc != kLargeOwnedBytes) {
        stress_fail_msg("owned read large rc failed");
    } else if (buffer == NULL || nm_io_buffer_capacity(buffer) < kLargeOwnedBytes) {
        stress_fail_msg("owned read large capacity failed");
    } else if (((const char *)nm_io_buffer_data(buffer))[0] != 'B' ||
               ((const char *)nm_io_buffer_data(buffer))[kLargeOwnedBytes - 1] != 'E') {
...
void run_dynamic_sleep_fanout(unsigned task_count, unsigned base_yields, uint64_t sleep_ns) {
    ...
    for (i = 0; i < task_count; ++i) {
        ...
        tasks[i] = nm_spawn(dynamic_sleep_child_task,
                            &child_states[i],
                            &(nm_spawn_opts_t){
                                .task_class = NM_TASK_CLASS_DEFAULT,
                                .stack_class = NM_STACK_CLASS_DEFAULT,
...
void run_recv_owned_peek_path(void) {
    ...
    rc = nm_recv_owned(sv[0], 16U, MSG_PEEK, &buffer);
    if (rc != (ssize_t)state.len) {
        stress_fail_msg("recv owned peek rc failed");
    } else if (buffer == NULL || memcmp(nm_io_buffer_data(buffer), state.payload, state.len) != 0) {
...
void run_recv_owned_multishot_path(void) {
    ...
    for (i = 0; i < (sizeof(payloads) / sizeof(payloads[0])); ++i) {
        size_t len = strlen(payloads[i]);
        ssize_t rc = nm_read_owned(sv[0], 16U, &buffer);

        if (rc != (ssize_t)len) {
            stress_fail_msg("recv owned multishot rc failed");
        } else if (buffer == NULL || memcmp(nm_io_buffer_data(buffer), payloads[i], len) != 0) {
...
void run_block_cancel_path(void) {
    ...
    waiter = nm_spawn(block_cancel_waiter_task,
                      &state,
                      &(nm_spawn_opts_t){
                          .task_class = NM_TASK_CLASS_DEFAULT,
                          .stack_class = NM_STACK_CLASS_DEFAULT,
    ...
    trigger = nm_spawn(block_cancel_trigger_task,
                       &state,
                       &(nm_spawn_opts_t){
                           .task_class = NM_TASK_CLASS_DEFAULT,
                           .stack_class = NM_STACK_CLASS_DEFAULT,
