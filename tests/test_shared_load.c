...
typedef int (*runtime_get_readiness_fn)(
    llam_runtime_t *runtime,
    llam_runtime_readiness_t *readiness,
    size_t readiness_size);
typedef int (*runtime_wake_fn)(llam_runtime_t *runtime);
...
#define LLAM_TEST_DEFAULT_SHARED_PATH "./libllam_runtime.so.2.2.0"
#else
...
int main(int argc, char **argv) {
    static const char *const llam_symbols[] = {
        ...
        "llam_channel_send",
        "llam_channel_try_send",
        "llam_channel_send_until",
        "llam_channel_recv_result",
        "llam_channel_try_recv_result",
        ...
        "llam_read_owned",
        "llam_recv_owned",
        "llam_io_buffer_opts_init",
        "llam_io_buffer_alloc_ex",
        "llam_io_buffer_alloc",
        "llam_io_buffer_alloc_aligned",
        "llam_io_buffer_alignment",
        "llam_pread_owned_aligned",
    ...
    runtime_wake_fn llam_runtime_wake_ptr = NULL;
    ...
    LOAD_FN(handle, "llam_runtime_wake", llam_runtime_wake_ptr);
    ...
    if (llam_runtime_opts_init_ptr(&opts, info.runtime_opts_size) != 0) {
        fprintf(stderr,
                "[test_shared_load] loaded llam_runtime_opts_init failed: errno=%d (%s)\n",
                errno,
                strerror(errno));
        (void)dlclose(handle);
        return 1;
