...
The current 2.1.0 implementation provides the following internal token and
...
- The local control transport exposes this setup as
  ...
  partially invalidated sessions still release their mapped resources. Reusing
  an inactive ring-session slot first reclaims any stale broker-owned mapping
  authority before the new session overwrites the slot.
...
- POSIX broker authority fds are made close-on-exec at every broker boundary:
  ...
  `accept4(SOCK_CLOEXEC)` on Linux, POSIX shm-backed ring fds use `O_CLOEXEC`,
  descriptor/ring duplicates use `F_DUPFD_CLOEXEC`, and received ancillary fds
  use `MSG_CMSG_CLOEXEC` before falling back to post-creation
  `fcntl(FD_CLOEXEC)`.
